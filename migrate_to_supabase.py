...
# Archivos relacionados con tablas
mapeo_archivos = {
...
def normalizar_envios_programados(data):
    result = []
    for d in data:
        result.append({
            "telefono": d.get("telefono"),
            "mensaje": d.get("mensaje"),
            "fecha_programada": d.get("fecha") or d.get("fecha_programada"),
...
def migrar():
    ...
    for archivo, tabla in mapeo_archivos.items():
        if archivo == "contactos.json":
            continue  # Ya migrado
        path = os.path.normpath(os.path.join(BASE, archivo))
        datos = cargar_json(path)

        # Correcciones específicas
        if tabla == "envios_programados":
            datos = normalizar_envios_programados(datos)
