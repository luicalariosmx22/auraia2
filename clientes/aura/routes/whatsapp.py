...
def obtener_contacto(numero_usuario):
    numero_usuario = normalizar_numero(numero_usuario)  # ✅ Normalizar

    try:
        response = supabase.table("contactos").select("*").eq("telefono", numero_usuario).execute()
        if not response.data:
            nuevo_contacto = {
                "telefono": numero_usuario,  # ✅ Usar campo correcto
                "nombre": "Desconocido",
                "ia_activada": True,
                "etiquetas": [],
...
def actualizar_contacto(numero_usuario, datos_actualizados):
    ...
    try:
        response = supabase.table("contactos").update(datos_actualizados).eq("telefono", numero_usuario).execute()
        if not response.data:
...
def webhook():
    ...
    numero_usuario = normalizar_numero(request.form.get('From'))  # ✅ Limpio
...
def enviar_mensaje():
    ...
    numero = normalizar_numero(data.get('numero'))  # ✅ Normalizar
