...
def panel_ads(nombre_nora):
    ...
    # Consultar todas las cuentas publicitarias para esta Nora
    cuentas_ads = supabase.table('meta_ads_cuentas').select('*').eq('nombre_visible', nombre_nora).execute().data or []

    # Revisar si se seleccionó una cuenta específica (por query param)
    ...
    if cuenta_id:
        ...
        if cuenta_info.data:
            ...
            # ✅ Pass date filters to the report function
